...
const CHECK_THRESHOLD = 180000; // 3 минуты
...
function handleRelayStateChange(isActive) {
    if (isActive) {
        ...
        // Проверяем, есть ли активная сессия для завершения
        if (sessionStartTime) {
            endCurrentSession();
        }
    }
...
function setupRealTimeListeners() {
    ...
    try {
        ...
        updateTotalTimeDisplay(calculateTotalTime(sessions));
        updateDisplay('totalTime', 'Общее время:', calculateTotalTime(sessions)); // Добавьте эту строку
    } catch (error) {
...
async function endCurrentSession() {
    try {
        const snapshot = await db.ref('currentSession').once('value');
        const startTime = snapshot.val();
        
        if (startTime && sessionStartTime) { // Добавлена проверка на sessionStartTime
            const duration = Math.floor((Date.now() - startTime) / 1000);
            await saveSession(startTime, duration);
            await db.ref('currentSession').remove();
            sessionStartTime = null; // Сбрасываем время сессии после сохранения
        }
    } catch (error) {
...
async function saveSession(startTime, duration) {
    try {
        await db.ref('sessions').transaction(sessions => {
            const newSessions = sessions || [];
            
            // Проверяем существование сессии по времени начала и длительности
            ...
            if (!exists) {
                newSessions.push({
                    ...
                    startTime,
                    timestamp: Date.now(),
                    hash: CryptoJS.MD5(`${startTime}-${duration}`).toString() // Добавляем хеш
                });
            }
            return newSessions;
        });
